...
namespace fc { namespace ip {
  ...
  bool address::is_private_address() const
  {
    if (is_ipv4()) {
      ...
      const auto& v6 = get_ipv6();
      if ((v6.addr.data[0] & 0xFE) == 0xFC) return true;                          // fc00::/7
      if (v6.addr.data[0] == 0xFE && (v6.addr.data[1] & 0xC0) == 0x80) return true;   // fe80::/10
      return false;
    }
  }
  ...
  bool address::is_multicast_address() const
  {
    if (is_ipv4()) {
      ...
      const auto& v6 = get_ipv6();
      return v6.addr.data[0] == 0xFF;  // ff00::/8
    }
  }
  ...
  //////////////////////////////////////////////////////////////////////////////
  // legacy_endpoint implementation
  //////////////////////////////////////////////////////////////////////////////

  legacy_endpoint::legacy_endpoint(const endpoint& ep)
  ...
  address from_asio_address(const boost::asio::ip::address& addr) {
    if (addr.is_v4()) {
      ...
      if (v6.is_v4_mapped())
        return address(boost::asio::ip::make_address_v4(boost::asio::ip::v4_mapped, v6).to_uint());
  ...
  {
    auto boost_addr = boost::asio::ip::make_address_v6(var.as_string().c_str());
...
namespace std
{
  ...
  size_t hash<fc::ip::address>::operator()(const fc::ip::address& a) const
  {
    if (a.is_ipv4()) {
      ...
      size_t seed = 0;
      for (int i = 0; i < 16; ++i) {
        seed ^= std::hash<uint8_t>()(v6.addr.data[i]) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
      }
      return seed;
    }
  }
