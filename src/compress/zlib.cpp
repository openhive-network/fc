...
namespace fc
{
  string zlib_compress(const string& in)
  {
    ...
    char* compressed_message = (char*)tdefl_compress_mem_to_heap(in.c_str(), in.size(), &compressed_message_length, static_cast<int>(TDEFL_WRITE_ZLIB_HEADER) | static_cast<int>(TDEFL_DEFAULT_MAX_PROBES));
