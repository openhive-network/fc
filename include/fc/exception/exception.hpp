...
#define FC_DECLARE_DERIVED_EXCEPTION( TYPE, BASE, CODE, WHAT ) \
  ...
  FC_DECLARE_EXCEPTION( sigint_exception, sigint_exception_code, "SIGINT Exception" );
...
#define FC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
    FC_MULTILINE_MACRO_BEGIN \
      if( UNLIKELY(!(TEST)) ) \
      {                                                                          \
        FC_ASSERT_EXCEPTION_DECL( #TEST, fc::assert_exception, "" __VA_ARGS__ )  \
...
#define FC_EXCEPTION_DECL( EXCEPTION_TYPE, OBJ_NAME, FORMAT, ... ) \
    EXCEPTION_TYPE OBJ_NAME( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) )
/**
