...
BOOST_AUTO_TEST_CASE(ipv4_address_via_address_string)
{
  // ipv4_address has no string constructor; construct via address
  fc::ip::address addr("192.168.1.1");
  BOOST_CHECK(addr.is_ipv4());
  BOOST_CHECK_EQUAL(addr.get_ipv4().addr, (192u << 24) | (168u << 16) | (1u << 8) | 1u);
}
...
{
  fc::ip::ipv6_address lo;
  lo.addr.data[15] = 1;
  // Verify through address string conversion
  fc::ip::address a(lo);
  BOOST_CHECK_EQUAL((std::string)a, "::1");
}

BOOST_AUTO_TEST_CASE(ipv6_from_address_string)
{
  // ipv6_address has no string constructor; construct via address
  fc::ip::address addr("::1");
  BOOST_CHECK(addr.is_ipv6());
  BOOST_CHECK_EQUAL(addr.get_ipv6().addr.data[15], 1);
  for (int i = 0; i < 15; ++i)
    BOOST_CHECK_EQUAL(addr.get_ipv6().addr.data[i], 0);
}
...
BOOST_AUTO_TEST_CASE(ipv6_full_address)
{
  ...
  BOOST_CHECK_EQUAL(v6.addr.data[0], 0x20);
  BOOST_CHECK_EQUAL(v6.addr.data[1], 0x01);
  BOOST_CHECK_EQUAL(v6.addr.data[2], 0x0d);
  BOOST_CHECK_EQUAL(v6.addr.data[3], 0xb8);
  BOOST_CHECK_EQUAL(v6.addr.data[15], 0x01);
}
...
{
  fc::ip::address a(fc::ip::ipv4_address(0x0A000001)); // 10.0.0.1
...
BOOST_AUTO_TEST_CASE(legacy_address_roundtrip)
{
  ...
  BOOST_CHECK_EQUAL((uint32_t)la, 0x01020304u);
...
BOOST_AUTO_TEST_CASE(legacy_address_from_address)
{
  ...
  BOOST_CHECK_EQUAL((uint32_t)la, 0xC0A80101u);
}
...
BOOST_AUTO_TEST_CASE(address_serialization_ipv4_roundtrip)
{
  ...
  auto packed = fc::raw::pack_to_vector(original);
  ...
  fc::raw::unpack_from_vector(packed, unpacked);
...
BOOST_AUTO_TEST_CASE(address_serialization_ipv6_roundtrip)
{
  ...
  BOOST_CHECK_EQUAL((std::string)unpacked, "2001:db8::1");
}
...
BOOST_AUTO_TEST_CASE(legacy_address_serialization_roundtrip)
{
  ...
  fc::raw::unpack_from_vector(packed, unpacked);
  BOOST_CHECK_EQUAL((uint32_t)unpacked, 0xC0A80101u);
}
...
BOOST_AUTO_TEST_CASE(legacy_endpoint_serialization_roundtrip)
{
  ...
  fc::raw::unpack_from_vector(packed, unpacked);
  BOOST_CHECK_EQUAL(unpacked.port(), 8080);
  BOOST_CHECK_EQUAL((uint32_t)unpacked.get_address(), 0x01020304u);
}
