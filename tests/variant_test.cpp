...
FC_AUTO_TEST_CASE( mutable_variant_object,
...
FC_AUTO_TEST_CASE( variant_object,
  fc::variant_object vo;
  BOOST_REQUIRE_EQUAL( vo.size(), 0 );
...
#define FC_TEST_VARIANT_FUNCTION_INVOKE(object, member_ptr) ((object).*(member_ptr))()
...
void test_type_conversion( variant_retrieve_member_fn_t< TypeTested > variant_retrieve_fn )
{
  ...
  const auto type_conversion_helper = [&]( auto&& input ) {
    v = input;
    BOOST_REQUIRE_EQUAL( (FC_TEST_VARIANT_FUNCTION_INVOKE( v, variant_retrieve_fn )), expected );
  };
  ...
  v.clear();
  BOOST_REQUIRE_EQUAL( (FC_TEST_VARIANT_FUNCTION_INVOKE( v, variant_retrieve_fn )), 0 );

  // Objects:
  v = fc::variant_object{};
  BOOST_REQUIRE_THROW( (FC_TEST_VARIANT_FUNCTION_INVOKE( v, variant_retrieve_fn )), fc::bad_cast_exception );
...
FC_AUTO_TEST_CASE( as_int64_conversions,
  test_type_conversion< int64_t >( &fc::variant::as_int64 );
)

FC_AUTO_TEST_CASE( as_uint64_conversions,
  test_type_conversion< uint64_t >( &fc::variant::as_uint64 );
)

FC_AUTO_TEST_CASE( as_double_conversions,
  test_type_conversion< double >( &fc::variant::as_double );
)

FC_AUTO_TEST_CASE( as_bool_conversions,
  bool expected = true;
  fc::variant v;
...
FC_AUTO_TEST_CASE( as_string_conversions,
...
FC_AUTO_TEST_CASE( as_blob_conversions,
  fc::blob blob;
  blob.data = { 'a', 'l', 'i', 'c', 'e' };
...
FC_AUTO_TEST_CASE( extended_variant_construction,
  using fc::variant;
  variant v;
...
FC_AUTO_TEST_CASE( from_variant_base,
...
FC_AUTO_TEST_CASE( to_variant_base,
...
FC_AUTO_TEST_CASE( variant_negation,
  BOOST_REQUIRE( !fc::variant{ false } );
  BOOST_REQUIRE( !fc::variant{ 0 } );
)

FC_AUTO_TEST_CASE( variant_compare_eq,
...
FC_AUTO_TEST_CASE( variant_compare_ne,
  BOOST_REQUIRE( !(fc::variant{ 1 } != fc::variant{ 1 }) );
  BOOST_REQUIRE( !(fc::variant{ "alice" } != fc::variant{ "alice" }) );
  BOOST_REQUIRE( (fc::variant{ 2 } != fc::variant{ 1 }) );
  BOOST_REQUIRE( (fc::variant{ true } != fc::variant{ 1 }) );
